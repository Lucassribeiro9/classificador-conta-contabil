...
try:
    ...
    empresa = Empresa(
        nome_empresa="Teste Contábil LTDA",
    ...
    treino_dados = [
        ("PAGAMENTO SALARIO MES 10", 101),
        ("SALARIO FUNCIONARIOS", 101),
        ("FOLHA DE PAGAMENTO", 101),
        ("VENDA MERCADORIA NF 55", 301),
        ("RECEBIMENTO CLIENTE A", 301),
        ("VENDA DE PRODUTOS", 301),
        ("PAGTO IMPOSTO SIMPLES", 202),
        ("GUIA DAS IMPOSTO", 202),
        ("IMPOSTO SOBRE FATURAMENTO", 202),
        ("PAGAMENTO SALARIO", 101),
        ("VENDA NF 123", 301),
        ("GUIA IMPOSTO RETIDO", 202),
    ]
    ...
    t3 = Transacao(
        empresa_id=empresa.id,
        historico="COMPRA CAFE ESCRITORIO",
        cod_banco=1,
        data=date.today(),
        valor=20.0,
    )  # Algo novo/incerto
    ...
    with open("test_result.log", "w", encoding="utf-8") as log:
        ...
        if sucesso:
            ...
            resultados = engine_ml.classify_transactions(
                empresa.id, ids_para_classificar
            )

            for res in resultados:
                revisao = "⚠️ REVISAR" if res.needs_review else "✅ OK"
                log.write(
                    f"Desc: {res.historico} -> Conta: {res.conta_contabil} | Confiança: {res.confidence:.2%} | Status: {revisao}\n"
                )
